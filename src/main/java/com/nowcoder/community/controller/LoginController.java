...
public class LoginController implements CommunityConstant {
    ...
    public String register(Model model, User user) {
        ...
        if (map == null || map.isEmpty()) {
            model.addAttribute("msg", "注册成功，我们已经向您得邮箱发送激活邮件");
    ...
    public String activation(Model model, @PathVariable("userId") int userId, @PathVariable("code") String code) {
        ...
        if (result == ACTIVATION_SUCCESS) {
            ...
            model.addAttribute("msg", "激活无效，该账号已被激活");
            model.addAttribute("target", "/index");
        } else {
    ...
    public String login(String username, String password, String code, boolean remember,
        ...
        //检查账号密码
        int expriedSecond = remember ? REMEMBER_EXPRIEDSECOND : DEFAULT_EXPRIEDSECOND;
        Map<String, Object> map = userService.login(username, password, expriedSecond);
        if (map.containsKey("ticket")) {
            Cookie cookie=new Cookie("ticket",map.get("ticket").toString());
            cookie.setPath(contextPath);
            cookie.setMaxAge(expriedSecond);
